(() => {
  ...
  const mini = root.querySelector('.apt-mini');
  ...
  const parseCustom = (raw) => {
    ...
    if ((p.length === 2 || p.length === 3) && p.every(x => /^\d+$/.test(x))) {
      ...
      if (p.length === 2) {
        if (a[1] >= 60) return null;
        return a[0] * 60 + a[1] || null;
      }
      if (a[1] >= 60 || a[2] >= 60) return null;
      return a[0] * 3600 + a[1] * 60 + a[2] || null;
    }
    return null;
  };
  ...
  const startLoop = () => {
    if (timerId) clearInterval(timerId);
    timerId = setInterval(updateRemaining, 250);
  };
