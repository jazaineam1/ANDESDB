...
function enhanceSession(s,m){const resources=[...(s.recursos||[])];if(Number(s.n)===15&&!resources.some(r=>String(r.href||'').includes('capstone.html')))resources.unshift({txt:'🎯 Proyecto final · datos, rúbrica y entrega',href:'capstone.html'});return {...s,recursos:resources,story_question:STORY[Number(s.n)]||'',estado_original:s.estado,estado:semanticStatus(s,m)}}
const normalize=m=>{if(!m||typeof m!=='object')return null;modules=(m.modulos||[]).map(mod=>({...mod,sesiones:(mod.sesiones||[]).map(s=>enhanceSession(s,m))}));sessions=modules.flatMap(mod=>(mod.sesiones||[]).map(s=>({...s,module_number:mod.n,module_title:mod.titulo,module_desc:mod.desc||''})));sessions.sort((a,b)=>Number(a.n)-Number(b.n));manifest={...m,grupo:Number(m.grupo||GROUP),conexion:m.conexion||{tipo:'Zoom',href:ZOOM},modulos:modules};return manifest};
...
function search(q){const x=fold(q).trim();if(!x)return [...sessions];return sessions.filter(s=>fold([s.n,s.titulo,s.desc,s.story_question,(s.tags||[]).join(' '),s.module_title,(s.recursos||[]).map(r=>r.txt).join(' ')].join(' ')).includes(x))}
