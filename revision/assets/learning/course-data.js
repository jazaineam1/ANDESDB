(()=>{
'use strict';
if(window.ANDES_COURSE?.version==='2.2.0')return;
...
function semanticStatus(s,m){
  const today=bogotaDate(),raw=String(s.estado||'pendiente');
  if(s.fecha===today)return'hoy';
  if(Number(s.n)===Number(m.sesionActual))return'actual';
  if(raw==='hoy'){
    if(s.fecha&&s.fecha<today)return'completado';
    if(s.fecha&&s.fecha>today)return'pendiente';
    return'actual';
  }
  return raw;
}
const normalize=m=>{
  ...
  modules=(m.modulos||[]).map(mod=>({...mod,sesiones:(mod.sesiones||[]).map(s=>({...s,estado_original:s.estado,estado:semanticStatus(s,m)}))}));
  ...
  manifest={...m,modulos:modules};return manifest;
};
...
let readyPromise;
if(manifest){readyPromise=Promise.resolve(manifest);queueMicrotask(()=>dispatchEvent(new CustomEvent('andesdb:course-ready',{detail:{manifest,sessions:[...sessions],cached:true}})));if(!cached?.saved_at||Date.now()-cached.saved_at>CACHE_MAX_AGE)setTimeout(refresh,0);else setTimeout(refresh,250)}else{readyPromise=refresh().then(x=>{dispatchEvent(new CustomEvent('andesdb:course-ready',{detail:{manifest:x,sessions:[...sessions],cached:false}}));return x}).catch(e=>{console.error('ANDESDB course manifest',e);return null})}
...
window.ANDES_COURSE={version:'2.2.0',ROOT,ready:()=>readyPromise,refresh,get manifest(){return manifest},get sessions(){return sessions},get modules(){return modules},session,search,upcoming,current,today,icsText,downloadCalendar};
})();