(()=>{
'use strict';
if(window.ANDES_COURSE?.version==='2.1.0')return;
...
const CACHE_KEY='andesdb.course.manifest.v3',CACHE_MAX_AGE=86400000;
...
async function refresh(){try{const r=await fetch(new URL('tools/curso.json',ROOT),{cache:'no-cache'});if(!r.ok)throw new Error('No se pudo cargar el manifiesto del curso');const next=await r.json();normalize(next);writeCache(next);dispatchEvent(new CustomEvent('andesdb:course-updated',{detail:{manifest:next,sessions:[...sessions]}}));return next}catch(e){console.warn('ANDESDB course manifest refresh',e);if(!manifest)throw e;return manifest}}
let readyPromise;
...
window.ANDES_COURSE={version:'2.1.0',ROOT,ready:()=>readyPromise,refresh,get manifest(){return manifest},get sessions(){return sessions},get modules(){return modules},session,search,upcoming,icsText,downloadCalendar};
})();