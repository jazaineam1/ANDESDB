...
function sqlView(t,i){return`<div class="task-instruction"><h2>${esc(t.title)}</h2><p>${esc(t.prompt)}</p></div><div class="sql-status" id="sql-load">El editor está listo. El motor SQL se prepara fuera de la interfaz.</div><label class="field-label" for="sql-editor">Tu SQL</label><textarea id="sql-editor" class="sql-editor" spellcheck="false" autocapitalize="off" autocomplete="off" autocorrect="off">${esc(st.draft[i]??t.starter??'')}</textarea><div class="actions"><button class="btn" id="run-btn">▶ Ejecutar</button><button class="btn good" id="verify-btn">✓ Ejecutar y validar</button>${t.hints?.length?'<button class="btn alt" id="hint-btn">Pista</button>':''}</div><div id="sql-result" class="resultbox"><div class="muted">Escribe con tranquilidad. La ejecución SQL ocurre en un proceso separado para no bloquear el teclado.</div></div>`}
...
function warmSQL(){const e=window.ANDES_SQL_ENGINE,status=$('sql-load');if(!e)return;if(e.isReady()){if(status)status.textContent='✓ Base lista · SQL se ejecuta sin bloquear el editor';return}const go=()=>{if(status)status.textContent='Preparando motor SQL en segundo plano…';e.prepare().then(()=>{if($('sql-load'))$('sql-load').textContent='✓ Base lista · SQL se ejecuta sin bloquear el editor'}).catch(err=>{if($('sql-load'))$('sql-load').textContent='El motor se preparará al ejecutar. '+String(err.message||'')})};if('requestIdleCallback'in window)requestIdleCallback(go,{timeout:1200});else setTimeout(go,350)}
...
function bindTask(t,i){
  if(t.type==='sql'){
    ...
    run.onclick=async()=>{attempt(i,{action:'run'});const sql=ed.value.trim();try{assertSafeSQL(sql);setBusy(run,true,'Ejecutando…');const r=await window.ANDES_SQL_ENGINE.run(sql);$('sql-result').innerHTML=table(r);clearFeedback()}catch(e){feedback(false,e.message)}finally{setBusy(run,false)}};
    verify.onclick=async()=>{attempt(i,{action:'verify'});const sql=ed.value.trim(),rev=editRevision;try{assertSafeSQL(sql);setBusy(verify,true,'Validando…');const r=await window.ANDES_SQL_ENGINE.validate(sql,t.reference);$('sql-result').innerHTML=table(r.got);if(rev!==editRevision){feedback(false,'La consulta cambió mientras se validaba. Vuelve a validar la versión actual.');return}if(r.match){feedback(true,'✓ Correcto. Columnas, filas, orden y valores coinciden con el objetivo.');await complete(i,{type:'sql'})}else{feedback(false,'Aún no coincide. Revisa columnas, filtros, orden y número de filas. Tu consulta se conserva.');fail(i,{type:'sql'})}}catch(e){feedback(false,e.message);fail(i,{type:'sql',error:String(e.message).slice(0,120)})}finally{setBusy(verify,false)}};
