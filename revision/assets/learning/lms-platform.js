...
/* El portal usa timeouts cortos. En móvil/cold-start una validación válida podía abortarse
   y presentarse como si el sistema no respondiera. Solo endurecemos learning-auth login/me;
   no reintentamos mutaciones académicas. */
if(!window.__ANDES_AUTH_FETCH_RESILIENCE__){
  ...
  window.fetch=async function(input,init={}){
    ...
    let action='';
    if(url.includes('/functions/v1/learning-auth')&&String(init?.method||'GET').toUpperCase()==='POST'){
      try{action=JSON.parse(init.body||'{}').action||''}catch(_){ }
    }
