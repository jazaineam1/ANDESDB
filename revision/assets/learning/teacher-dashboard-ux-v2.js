...
.credentials[data-mail-enhanced="1"]{border-left:4px solid var(--andes-blue,#234a73)!important}.credential-delivery-note{margin-top:7px;color:var(--andes-muted,var(--muted));font-size:.76rem;line-height:1.45}.credentials .act.mail{background:var(--andes-blue,#234a73)!important;color:#fff!important;text-decoration:none}
...
function enhanceCredentials(){document.querySelectorAll('.credentials:not([data-mail-enhanced])').forEach(box=>{const data=credentialMessage(box);if(!data)return;box.dataset.mailEnhanced='1';const actions=box.querySelector('.actions')||(()=>{const d=document.createElement('div');d.className='actions';box.appendChild(d);return d})();const a=document.createElement('a');a.className='act mail';a.dataset.mailCredentials='1';a.textContent=lastRecipientEmail?'Enviar por correo':'Abrir correo';const subject='Acceso temporal a ANDESDB';a.href=`mailto:${lastRecipientEmail&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(lastRecipientEmail)?lastRecipientEmail:''}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(data.text)}`;actions.appendChild(a);const copy=document.createElement('button');copy.type='button';copy.className='act alt';copy.textContent='Copiar mensaje';copy.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(data.text);copy.textContent='Mensaje copiado ✓'}catch{toast('No pude copiar el mensaje. Usa “Abrir correo”.','err')}});actions.appendChild(copy);const note=document.createElement('div');note.className='credential-delivery-note';note.textContent=lastRecipientEmail?`Destino preparado: ${lastRecipientEmail}. Revisa el mensaje antes de enviarlo.`:'Se abrirá tu aplicación de correo con usuario, contraseña temporal y enlace al portal. Revisa el destinatario antes de enviar.';box.appendChild(note)})}
