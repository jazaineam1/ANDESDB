...
.assignment-card{display:grid!important;grid-template-columns:minmax(0,1fr) auto;gap:16px;align-items:center;padding:16px!important;background:var(--andes-paper,#fff)!important;border:1px solid var(--andes-line,#d7dadd)!important;border-left:4px solid var(--andes-gold,#f2c300)!important;border-radius:6px!important;box-shadow:none!important}
...
#andes-archive-dialog{width:min(520px,92vw);border:0;border-radius:8px;padding:0;background:var(--andes-paper,#fff);color:var(--andes-ink,#171717);box-shadow:0 30px 90px rgba(0,0,0,.34)}
...
.teacher-toast{position:fixed;right:18px;bottom:18px;z-index:1000;width:min(420px,calc(100vw - 36px));background:var(--andes-nav,#111820);color:#fff;border-left:5px solid var(--andes-gold,#f2c300);border-radius:6px;padding:13px 15px;box-shadow:0 18px 44px rgba(0,0,0,.28);font:750 .86rem/1.4 system-ui;opacity:0;transform:translateY(8px);pointer-events:none;transition:.18s ease}
.teacher-toast.show{opacity:1;transform:none}.teacher-toast.ok{border-left-color:#54c493}.teacher-toast.err{border-left-color:#ff9c94}
#content>.cols>.card:first-child{border-top:5px solid var(--andes-gold,#f2c300)!important}
#content>.card .ey,#content>.cols>.card .ey{margin-bottom:4px}
@media(max-width:680px){.assignment-card{grid-template-columns:1fr;padding:14px!important}.assignment-card [data-archive]{justify-self:stretch;width:100%}.archive-dialog-actions{display:grid;grid-template-columns:1fr 1fr}.archive-dialog-actions button{width:100%}.teacher-toast{bottom:84px;right:14px;width:calc(100vw - 28px)}}
`;
...
injectStyle();
document.addEventListener('click',e=>{const btn=e.target.closest?.('[data-archive]');if(!btn)return;e.preventDefault();e.stopImmediatePropagation();archive(btn)},true);
