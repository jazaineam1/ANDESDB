const VERSION='andesdb-lms-20260912-v32';
...
const ESSENTIAL=[
  ...
  './assets/learning/presentation-performance.js','./assets/learning/presentation-resume.js','./assets/learning/presentation-lms-sync.js','./assets/learning/session-lab-button.js','./assets/learning/interactive-nav.js','./assets/learning/interactive-nav-base.js'
];
...
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET')return;const url=new URL(req.url);if(!allowed(url))return;
  ...
  const critical=/\/assets\/learning\/(?:presentation-performance|presentation-resume|presentation-lms-sync|session-lab-button|interactive-nav|interactive-nav-base|learning-tracker|learning-tracker-v3|access-gate)\.js$/i.test(url.pathname);
