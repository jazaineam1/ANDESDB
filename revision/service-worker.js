const VERSION='andesdb-lms-20260912-v38';
...
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET')return;const url=new URL(req.url);if(!allowed(url))return;
  ...
  const critical=/\/assets\/learning\/(?:interactive-nav|access-gate|resource-dock-a11y|presentation-text-fixes|presentation-telemetry|presentation-resume|lab-runtime-v4|lab-runtime-v5|lab-capstone-patch|lab-mcq-v1|lab-ux-v6-patch|lms-platform|lms-ux-v1|learning-tracker)\.js$/i.test(url.pathname);
