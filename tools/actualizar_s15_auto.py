...
def update_course()->None:
    ...
    s.update({
      ...
      'desc':'Evaluación integradora de S2–S14: SQL, modelado y 3FN, DDL, documentos, Data Warehouse, BigQuery físico, ARRAY/STRUCT/UNNEST y Boss de transferencia. Dominio corregible 80 puntos + transferencia inédita 20 puntos; nota recalculada server-side.',
      'tags':['evaluación automática','SQL','normalización','DDL','NoSQL','Data Warehouse','BigQuery','partition','clustering','ARRAY','STRUCT','UNNEST','transferencia','server-side grader'],
...
def update_plan()->None:
    ...
    s.update({
      ...
      'objetivo':'Integrar S2–S14 en una solución ejecutable y transferir el razonamiento a un caso nuevo.',
      ...
      'reto':{'titulo':'Boss Transfer · 20 puntos','instrucciones':'Resuelve pedidos omnicanal con un caso nuevo; el servidor además ejecuta pruebas ocultas de DDL y SQL.','criterios':['grano línea de pedido','medidas cantidad e importe','partición por fecha','clustering por patrón de filtros','UNNEST de items sin perder pedido_id']},
...
def sync()->None:
    for rel in (
      ...
      'assets/learning/s15-autograder-v6.js','assets/learning/s15-autograder-v6b.js','assets/learning/s15-workbench-v6.css',
...
def main()->int:
    update_course(); update_plan(); sync(); print('S15 v6 sincronizada: S2–S14 + Boss, mastery 80 + transfer 20, grader server-side.'); return 0
