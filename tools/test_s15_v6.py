...
def test_q2_mutant_killed():
    ...
    mutant="""SELECT c.caso_id,c.estado AS estado_actual,MAX(e.estado) AS ultimo_estado FROM casos_src c LEFT JOIN eventos_src e ON e.caso_id=c.caso_id GROUP BY c.caso_id,c.estado"""
    correct="""WITH ult AS (SELECT caso_id,estado,ROW_NUMBER() OVER(PARTITION BY caso_id ORDER BY fecha_evento DESC,evento_id DESC) rn FROM eventos_src) SELECT c.caso_id,c.estado,u.estado FROM casos_src c LEFT JOIN ult u ON u.caso_id=c.caso_id AND u.rn=1"""
    m=c.execute(mutant).fetchall();g=c.execute(correct).fetchall();c.close();assert m!=g,'MAX(estado) volvió a sobrevivir al dataset visible'

def test_interface_contract():
    html=(ROOT/'evaluador-s15-v6.html').read_text(encoding='utf-8');js=(ROOT/'assets/learning/s15-autograder-v6b.js').read_text(encoding='utf-8');css=(ROOT/'assets/learning/s15-workbench-v6.css').read_text(encoding='utf-8')
    ...
    for token in ('PARTITION BY','CLUSTER BY','UNNEST','ARRAY&lt;STRUCT&gt;','Parquet','OLTP','OLAP','Mutation Hunter'):assert token in html,token
    assert 'type="radio"' not in html.lower()
    assert "VERSION='s15-workbench-v6'" in js
    assert "reference:" not in js.lower(),'No publicar queries de referencia finales'
    assert 'v3.casos' in js and "E099" in js,'Falta contraejemplo visible temporal'
    assert 'bqEstimate' in js and 'unnestOk' in js and 'dwScore' in js
    assert 'data-drop' in html and 'dragstart' in js and 'touch' in css
    assert 'min-height:44px' in css and 'font-size:16px' in css

def test_stable_routes():
    ev=(ROOT/'evaluador-s15.html').read_text(encoding='utf-8');pr=(ROOT/'Presentaciones/M6/sesion-15-desafio-final.html').read_text(encoding='utf-8')
    assert 'evaluador-s15-v6.html' in ev and 'v5.html' not in ev
    assert 'sesion-15-desafio-final-v6.html' in pr and 'v5.html' not in pr

def test_presentation():
    p=(ROOT/'Presentaciones/M6/sesion-15-desafio-final-v6.html').read_text(encoding='utf-8')
    assert p.count('class="slide')==14
    no_scripts=re.sub(r'<script[\s\S]*?</script>','',p,flags=re.I)
    mins=re.findall(r'\b\d+\s*(?:minutos|min)\b',no_scripts,flags=re.I)
    assert mins==['15 minutos','15 minutos'],f'Minutaje visible inesperado: {mins}'
    for t in ('Data Warehouse','BigQuery','ARRAY','STRUCT','UNNEST','Boss'):assert t in p
...
def test_manifest_contract():
    c=json.loads((ROOT/'tools/curso.json').read_text(encoding='utf-8'));s=next(s for m in c['modulos'] for s in m['sesiones'] if s['n']==15)
